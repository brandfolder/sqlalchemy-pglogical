...
def make_func(ddl):
    ...
    return lambda element, compiler, **kwargs: ddl_to_replicate_command(getattr(compiler, visit_name).__call__(element, **kwargs))


# get every DDLElement that can be extended the way we expect (the hueristic here is having a `__visit_name__`)
replicable_ddls = [ddl for ddl in all_subclasses(DDLElement) if hasattr(ddl, '__visit_name__')]
